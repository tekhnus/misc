...
def dfs_iterative(gf):
    ...
    while st:
        ...
        if v in visited:
            ...
            yield ("enter", e, u, v)
            visited.add(v)
            st.append(collections.deque((e, v, w) for e, w in gf(v)))
    ...
    while (path := find_path_bfs(t, traverse(outbound_edges_filtered_by_weight, [s]))) is not None:
        ...
        for e, u, v in path:
            ...
            if not is_backward(e):
                used_wg[oe] += pathwgh
                rest_bwd.edges_update([(original_eid(e), v, u)])
            else:
                used_wg[oe] -= pathwgh
    return wgh, used_wg
