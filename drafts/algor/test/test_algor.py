...
def test_dfs_2(gr, exp, f):
    exp = [(label, tuple(rest)) for label, *rest in exp]
...
@pytest.mark.parametrize("gr,exp", [(g1, [4, 3, 2, 1])])
def test_topo_sort(gr, exp):
    assert list(algor.topo_sort(gr.outbound_edges, 1)) == exp
