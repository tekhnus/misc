...
main = hspec $ do
    describe "perft" $ do
        ...
        it "handles the position #5 from wiki" $
             perftLengths "rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8" [1..3] `shouldBe` [44, 1486, 62379]
        it "handles the position #2 from wiki" $
             perftLengths "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq -" [1..3] `shouldBe` [48, 2039, 97862]
        it "handles a position where an immediate enpassant is possible" $
             perftLengths "rnbqkbnr/pp11pppp/8/2ppP3/8/8/PPPP1PPP/RNBQKBNR w KQkq d6" [1..1] `shouldBe` [31]
