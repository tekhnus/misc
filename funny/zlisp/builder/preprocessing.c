...
EXPORT struct expander_state expander_state_make() {
  ...
  result init_res = routine_run_with_handler(e.expander_sl, &e.expander_routine, host_ffi);
...
EXPORT fdatum datum_expand(datum *e, struct expander_state *est) {
  ...
  result res = routine_run_with_handler(est->expander_sl, &est->expander_routine, host_ffi);
