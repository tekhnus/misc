...
pointer-call-and-deserialize :=
{fn {fn-ptr signature params}
 {fnparamst :=
  (../head signature)
  rettype :=
  (../head (../tail signature))
  s :=
  (../serialize-params params fnparamst)
  rawres :=
  (../pointer-call fn-ptr {list
    {fnparamst
     rettype}} s)
...
c-data-pointer :=
{fn {handle c-name signature}
 {fn-pointer-pointer :=
  (../dlsym handle c-name)
  fn-pointer :=
  (../dereference fn-pointer-pointer 'int64)
  return fn-pointer}}
...
annotate-pointer :=
(dereference (dlsym selflib "builtin_annotate") 'int64)
...
is-constant-pointer :=
(dereference (dlsym selflib "builtin_is_constant") 'int64)
...
repr-pointer :=
(dereference (dlsym selflib "builtin_repr") 'int64)
...
(dereference (dlsym selflib "builtin_concat_bytestrings") 'int64)
...
+-pointer :=
(dereference (dlsym selflib "builtin_add") 'int64)
...
{fn {handle c-name signature}
 {res :=
  (../c-data-pointer handle c-name signature)
  if (../eq 0 res)
