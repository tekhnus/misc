...
compdata-make = (/prelude/c-function selflib "compdata_alloc_make" {list {quote {() pointer}}})
make-routine-with-empty-state = (/prelude/c-function selflib "routine_make_alloc" {list {quote {(sizet pointer) pointer}}})
prog-slice-make = (/prelude/c-function selflib "vec_create_slice" {list {quote {() progslice}}})
prog-build-one-c-host = (/prelude/c-function buildlib "prog_build" {list {quote {(pointer pointer pointer pointer pointer pointer pointer) pointer}}})
prog-build-init = (/prelude/c-function buildlib "prog_build_init" {list {quote {(pointer pointer pointer) sizet}}})
get-host-ffi-settings = (/prelude/c-function buildlib "get_host_ffi_settings" {list {quote {() pointer}}})
ext-make = (/prelude/c-function buildlib "standard_extension_alloc_make" {list {quote {() pointer}}})
...
{e = (/prelude/prog-build-one-c-host (/prelude/wrap-pointer-into-pointer sl) (/prelude/wrap-pointer-into-pointer bpptr) (/prelude/wrap-pointer-into-pointer src) compdata bdrcompdata (/prelude/get-host-ffi-settings) ex)
 {if (/std/eq 0 (/prelude/dereference e {quote {int64}}))
  {return `(:ok :nothing)}
  {return `(:err ~(/prelude/dereference e {quote {string}}))}}}

routine-run-and-get-value-c-host-new = (/prelude/c-function selflib "routine_run_in_ffi_host" {list {quote {(progslice pointer) fdatum}}})
fdatum-is-panic = (/prelude/c-function selflib "fdatum_is_panic" {list {quote {(fdatum) int}}})
fdatum-get-value-ptr = (/prelude/dlsym selflib "fdatum_get_value")
defn fdatum-get-value {x}
{return (/prelude/call-extension-1 (/prelude/dereference fdatum-get-value-ptr {quote {int64}}) x)}

fdatum-get-panic-message-ptr = (/prelude/dlsym selflib "fdatum_get_panic_message")
defn fdatum-get-panic-message {x}
{return (/prelude/call-extension-1 (/prelude/dereference fdatum-get-panic-message-ptr {quote {int64}}) x)}

fdatum-repr-datum-pointer-ptr = (/prelude/dlsym selflib "fdatum_repr_datum_pointer")
defn repr-pointer {x}
{return (/prelude/call-extension-1 (/prelude/dereference fdatum-repr-datum-pointer-ptr {quote {int64}}) x)}
...
datum-read-one = (/prelude/c-function selflib "datum_read_one" {list {quote {(pointer) fdatum}}})
defn read {strm}
{res = (/prelude/datum-read-one strm)
 {if (/std/eq (/prelude/fdatum-is-panic res) 1)
  {msg = (../fdatum-get-panic-message res)
   {if (/std/eq msg "eof")
    {return {list {quote {:eof}}}}
    {return `(:err ~msg)}}}
