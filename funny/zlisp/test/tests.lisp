...
fntest {defn far-fib () {{return @0 @event-loop 3} {return @0 @event-loop 5} {return @0 8}} defn more-far-fib () {x = (../far-fib) {return @0 @event-loop x} {return @0 @event-loop 13}} (x) = (more-far-fib @mut @event-loop) (y) = (more-far-fib @mut @event-loop) (z) = (more-far-fib @mut @event-loop) (t) = (more-far-fib @mut @event-loop) {return `(~x ~y ~z ~t)}} '(3 5 8 13)
...
fntest {defn cl-holder (x xs) {() = (return @0) () = (return @0 x xs)} defn cl-cons (x xs) {holder = cl-holder () = (../holder @0 @mut x xs) {return holder}} defn cl-head (xs) {(h r) = (../xs @2) {return h}} defn cl-tail (xs) {(h r) = (../xs @2) {return r}} cl-nil = :nil xs0 = cl-nil xs1 = (cl-cons 42 xs0) xs2 = (cl-cons 34 xs1) a = (cl-head xs2) b = (cl-head (cl-tail xs2)) {return `(~a ~b)}} '(34 42)
