...
fdatum builtin_annotate(datum *args) {
  ...
  return fdatum_make_ok(
      *datum_make_list_of(datum_make_list_of(datum_make_symbol(type), arg_value)));
}
...
fdatum builtin_cons(datum *args) {
  ...
  datum e = *datum_make_list_of(head);
...
fdatum builtin_head(datum *args) {
  ...
  return fdatum_make_ok(*datum_make_list_of(list_at(list, 0)));
}
